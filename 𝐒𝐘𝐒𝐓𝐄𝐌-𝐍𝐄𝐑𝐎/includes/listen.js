module.exports = function({ api, models }) {
  ...
  const checkttDataPath = __dirname + '/../𝐍𝐄𝐑𝐎-𝐒𝐂𝐑𝐈𝐏𝐓/𝐂𝐎𝐌𝐌𝐀𝐍𝐃𝐒/checktt/';
  setInterval(async() => {
...
(async function () {
  ...
  //DEFINE DATLICH PATH
  const datlichPath = __dirname + '/../𝐍𝐄𝐑𝐎-𝐒𝐂𝐑𝐈𝐏𝐓/𝐂𝐎𝐌𝐌𝐀𝐍𝐃𝐒/cache/datlich.json';
    ...
    for (el of temp) {
      ...
      if ("ATTACHMENT" in el) {
        ...
        for (a of el.ATTACHMENT) {
          ...
          fs.writeFileSync(__dirname + `../../𝐍𝐄𝐑𝐎-𝐒𝐂𝐑𝐈𝐏𝐓/𝐂𝐎𝐌𝐌𝐀𝐍𝐃𝐒/cache/${a.fileName}`, Buffer.from(getAttachment, 'utf-8'));
          out.attachment.push(fs.createReadStream(__dirname + `/../𝐍𝐄𝐑𝐎-𝐒𝐂𝐑𝐈𝐏𝐓/𝐂𝐎𝐌𝐌𝐀𝐍𝐃𝐒/cache/${a.fileName}`));
        }
      }
      console.log(out);
      if ("BOX" in el) await api.setTitle(el["BOX"], el["TID"]);
      api.sendMessage(out, el["TID"], () => ("ATTACHMENT" in el) ? el.ATTACHMENT.forEach(a => fs.unlinkSync(__dirname + `../../𝐍𝐄𝐑𝐎-𝐒𝐂𝐑𝐈𝐏𝐓/𝐂𝐎𝐌𝐌𝐀𝐍𝐃𝐒/cache/${a.fileName}`)) : "");
    }
...
/*
  return (event) => {
    ...
    let data = JSON.parse(fs.readFileSync(__dirname + "./../𝐍𝐄𝐑𝐎-𝐒𝐂𝐑𝐈𝐏𝐓/𝐂𝐎𝐌𝐌𝐀𝐍𝐃𝐒/cache/approvedThreads.json"));
    ...
    if (!data.includes(event.threadID) && !adminBot.includes(event.senderID)) {
      ...
      if (event.body && event.body == `${prefix}طلب`) {
        ...
        return api.sendMessage(`✅ سـيـتـم طـلـب الـمـوافـقـه المطور على مجموعتك ✅\n\n «https://www.facebook.com/YASSINEANBU/»`, event.threadID);
      }
      if (event.body && event.body.startsWith(prefix)) return api.sendMessage(`⚠️ 𝐍𝐄𝐑𝐎 𝐁𝐎𝐓 𝐖𝐀𝐑𝐍𝐈𝐍𝐆 ⚠️\n
❌| الـمـجـمـوعـة لـم تـؤخـد اذن الـمـطـور \n
📛| الـمـرجـو طـرد بـوت مـن الـمـجـمـوعـة\n
✅| او اكـتـب  (.طلب)   لـي يـتـم ارسـال طـلـب الـمـوافـقـة الـى الـمـطـور\n 
📨| لـي مـزيـد مـن مـعـلـومـات راسـل الـمـطـور :  https://www.facebook.com/YASSINEANBU`, event.threadID);
    };
    */
  return (event) => {
    switch (event.type) {
      ...
      case "message_reaction":
        ...
        if (event.reaction === "🤔") {
          const replyMessage = `ايدي: ${event.senderID}`;
           api.sendMessage(replyMessage, event.threadID, event.messageID);
           }                 
        if (event.reaction === "😐" && event.userID === "100034144836882" ) { 
