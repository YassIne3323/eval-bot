module.exports = function ({ api, models, Users, Threads, Currencies, globalData, usersData, threadsData, Nero }) {
  ...
  return async function ({ event }) {
    ...
    // تحقق مما إذا كان الأمر يتطلب البادئة
    ...
    if (!command) {
      ...
      if (checker.bestMatch.rating >= 0.8) {
        ...
        api.sendMessage(`الأمـر ${commandName} غـيـر مـوجـود 😿✘\n هـل تـقـصـد '${closestMatch}'🐦‍⬛?`, threadID, messageID);
        return;
      }
    }

    // تحقق مما إذا كان يجب استخدام البادئة لهذا الأمر
    if (command.config.usePrefix !== false && !prefixRegex.test(body)) return;
    ...
    try {
      const Obj = {
        api, event, args, models, Users, Nero, usersData, threadsData, globalData, Threads, Currencies, permssion, getText: getText2
      };
      command.run(Obj);
